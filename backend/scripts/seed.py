...
def seed() -> None:  # noqa: PLR0915 - a linear seed script reads best top-to-bottom
    ...
    try:
        ...
        make_user(
            db,
            settings.first_viewer_email,
            "Read-Only Viewer",
            Role.viewer,
            settings.first_viewer_password,
            mfa=False,
        )
        marco = make_user(
            db, "marco.sre@sentryops.local", "Marco (SRE)", Role.operator, "demo12345", True
        )
        dana = make_user(
            db, "dana.ops@sentryops.local", "Dana (Ops)", Role.operator, "demo12345", False
        )
        priya = make_user(
            db, "priya.sec@sentryops.local", "Priya (Security)", Role.admin, "demo12345", True
        )
        db.flush()
        ...
        spec: list[tuple] = [
            ...
            (
                ...
                "us-east-1",
                ["prod-critical", "pci"],
                {"tls_cert_expires_at": days_ahead(9), "encryption_in_transit": False},
            ),
            (
                "search-es",
                AssetType.service,
            ...
            (
                ...
                "dana",
                "us-east-1",
                ["prod-critical", "database", "pci"],
                {"open_ports": [5432, 22, 3389], "backup_last_at": days_ago(1)},
            ),
            (
                "postgres-replica",
                AssetType.service,
            ...
            (
                ...
                "saas",
                ["internal"],
                {"edr_present": False, "antivirus_present": False, "disk_encryption": False},
            ),
            (
                "office365-tenant",
                AssetType.software_license,
            ...
            (
                ...
                "us-east-1",
                ["internal"],
                {"firewall_enabled": False, "logging_enabled": False},
            ),
        ]
        ...
        edges = [
            ("web-app-01", "postgres-primary"),
            ("web-app-01", "redis-cache"),
            ("web-app-01", "auth-service"),
            ("web-app-02", "postgres-primary"),
            ("web-app-02", "redis-cache"),
            ("web-app-02", "auth-service"),
            ("api-gateway-01", "web-app-01"),
            ("api-gateway-01", "web-app-02"),
            ("api-gateway-01", "cdn-edge"),
            ...
            ("worker-01", "rabbitmq"),
            ("worker-01", "postgres-primary"),
            ("worker-02", "rabbitmq"),
            ("search-es", "object-store"),
            ("rds-analytics", "postgres-replica"),
            ("grafana", "prometheus"),
            ("web-app-01", "k8s-prod-cluster"),
            ("billing-service", "k8s-prod-cluster"),
        ]
        ...
        for sname, asset_name, slo, url in svc_spec:
            svc = Service(
                name=sname,
                description=f"{sname} synthetic monitor",
        ...
        db.add(
            AuditLog(
                ...
                entity_type="asset",
                entity_id=assets["postgres-primary"].id,
                source_ip="10.0.4.21",
        ...
        db.add(
            AuditLog(
                ...
                entity_type="asset",
                entity_id=assets["billing-service"].id,
                source_ip="10.0.4.30",
                before={"lifecycle_state": "maintenance"},
        ...
        billing = Incident(
            service_id=services["Billing Service"].id,
            health_check_id=checks["Billing Service"].id,
            asset_id=assets["billing-service"].id,
            title="Billing Service is failing health checks",
            status=IncidentStatus.open,
            severity=Severity.critical,
            opened_at=NOW - timedelta(hours=2, minutes=10),
        )
        db.add(billing)
        db.flush()
        event(
            ...
            "Opened after 3 consecutive failed checks.",
            NOW - timedelta(hours=2, minutes=10),
        )

        # 2) RESOLVED — Search (feeds MTTA/MTTR)
        search = Incident(
            service_id=services["Search"].id,
            health_check_id=checks["Search"].id,
            asset_id=assets["search-es"].id,
            title="Search latency breached SLO",
            status=IncidentStatus.closed,
            severity=Severity.high,
            opened_at=NOW - timedelta(days=3),
            acknowledged_at=NOW - timedelta(days=3) + timedelta(minutes=9),
            acknowledged_by_id=operator.id,
            resolved_at=NOW - timedelta(days=3) + timedelta(minutes=52),
            resolved_by_id=operator.id,
            closed_at=NOW - timedelta(days=3) + timedelta(minutes=52),
        )
        db.add(search)
        db.flush()
        event(
            search,
            IncidentEventType.opened,
            "Opened after sustained latency over budget.",
            search.opened_at,
        )
        event(
            search,
            ...
            search.acknowledged_at,
            operator,
        )
        event(
            search,
            IncidentEventType.comment,
            "Rolling index merge throttled; raising heap.",
            search.acknowledged_at + timedelta(minutes=14),
            operator,
        )
        event(
            search,
            IncidentEventType.resolved,
            "Latency recovered; closing.",
            search.resolved_at,
            operator,
        )
        event(
            ...
            "Check green for 2 consecutive runs.",
            search.closed_at,
        )

        # 3) RESOLVED — Auth blip (feeds MTTA/MTTR)
        auth = Incident(
            service_id=services["Auth Service"].id,
            health_check_id=checks["Auth Service"].id,
            asset_id=assets["auth-service"].id,
            title="Auth Service brief 5xx spike",
            status=IncidentStatus.closed,
            severity=Severity.medium,
            opened_at=NOW - timedelta(hours=26),
            acknowledged_at=NOW - timedelta(hours=26) + timedelta(minutes=4),
            acknowledged_by_id=marco.id,
            resolved_at=NOW - timedelta(hours=26) + timedelta(minutes=23),
            resolved_by_id=marco.id,
            closed_at=NOW - timedelta(hours=26) + timedelta(minutes=23),
        )
        db.add(auth)
        db.flush()
        ...
        event(
            auth,
            IncidentEventType.resolved,
            "Transient pod eviction; recovered.",
            auth.resolved_at,
            marco,
        )
                ...
                "remediation_steps": [
                    {
                        ...
                        "rationale": "The outage correlates tightly with that change; reverting is the fastest path to recovery.",
                        "priority": 1,
                    },
                    {
                        "step": "Restart billing-service workers to clear stuck/exhausted connections.",
                        "rationale": "Clears stale pooled connections after the upstream fix.",
                        "priority": 2,
                    },
                    {
                        "step": "Verify postgres-primary health (active connections, replication lag) before resolving.",
                        "rationale": "Rules out a deeper database fault.",
                        "priority": 3,
                    },
                ],
        ...
        event(
            billing,
            IncidentEventType.ai_triaged,
            "AI triage completed (illustrative).",
            NOW - timedelta(hours=2, minutes=8),
        )
        ...
        seed_triage(
            ...
            search,
            {
                ...
                "remediation_steps": [
                    {
                        "step": "Throttle index merges and raise JVM heap on search-es.",
                        "rationale": "Relieves GC pressure driving latency.",
                        "priority": 1,
                    },
                    {
                        "step": "Add a queue-depth alert ahead of SLO breach.",
                        "rationale": "Earlier detection next time.",
                        "priority": 2,
                    },
                ],
        ...
        for d, score, sev in history:
            ...
            db.add(
                ComplianceRun(
                    ...
                    triggered_by_id=priya.id,
                    total_assets=len(assets),
                    org_score=score,
                    passed_count=0,
                    failed_count=sum(sev.values()),
                    not_applicable_count=0,
                    severity_failing=sev,
                )
            )
        db.commit()
